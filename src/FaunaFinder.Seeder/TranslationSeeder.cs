...
public static class TranslationSeeder
{
    public static async Task SeedAsync(FaunaFinderContext context, CancellationToken cancellationToken = default)
    {
        ...
        // === LANGUAGES ===
        var english = new Language { Id = 0, Code = "en", Name = "English", IsDefault = true };
        var spanish = new Language { Id = 0, Code = "es", Name = "Español", IsDefault = false };

        context.Languages.AddRange(english, spanish);
        ...
        foreach (var (key, value) in EnglishTranslations)
        {
        ...
        foreach (var (key, value) in SpanishTranslations)
        {
            translations.Add(new Translation
            {
                Id = 0,
    ...
    private static readonly Dictionary<string, string> SpanishTranslations = new()
    {
        // Navigation
        ["Nav_Map"] = "Mapa",
