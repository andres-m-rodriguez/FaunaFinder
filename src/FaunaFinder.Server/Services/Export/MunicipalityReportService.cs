...
public class MunicipalityReportService(IAppLocalizer localizer) : IMunicipalityReportService
{
    ...
    public byte[] GeneratePdfReport(
        ...
        IReadOnlyList<SpeciesForListDto> species
    )
    {
    ...
    private void ComposeHeader(IContainer container, string municipalityName)
    {
        container.Column(column =>
        {
                ...
                .Row(row =>
                {
                    row.RelativeItem()
                        .Column(col =>
                        {
                            ...
                            col.Item()
                                .Text("Puerto Rico Conservation Report")
                                .FontSize(12)
            ...
            column.Item().PaddingTop(15).Text(municipalityName).Bold().FontSize(16);
    ...
    private void ComposeContent(IContainer container, IReadOnlyList<SpeciesForListDto> species)
    {
        container.Column(column =>
        {
            column
                .Item()
                .Text($"Total Species: {species.Count}")
                .FontSize(11)
                .Bold()
    ...
    )
    {
        ...
        // Header row
        sb.AppendLine(
            "Municipality,Common Name,Scientific Name,NRCS Practice Code,NRCS Practice Name,FWS Action Code,FWS Action Name,Justification"
        );
        ...
        foreach (var s in species)
        {
            if (s.FwsLinks.Count == 0)
            {
                ...
                sb.AppendLine(
                    $"{EscapeCsv(municipalityName)},{EscapeCsv(_localizer.GetLocalizedValue(s.CommonName))},{EscapeCsv(s.ScientificName)},,,,"
                );
            }
            else
            {
